...
export async function getUserEmail(userId, accessToken) {
  ...
  let res = await client
    .api("/users/25d251d7-f5c3-4f60-b9aa-72f079d32ba1/messages")
    .select("sender,subject")
    .get();
}

export async function getUserEvents(userId, accessToken) {
  const client = graph.Client.init({
    authProvider: (done) => {
      done(null, accessToken.accessToken);
    },
  ...
  let res = await client
    .api("/users/25d251d7-f5c3-4f60-b9aa-72f079d32ba1/events")
